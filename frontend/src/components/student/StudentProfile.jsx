...
function StudentProfile() {
    ...
    const [detailsRequested, setDetailsRequested] = useState(false); // State to track whether update was requested
    ...
    const loadStudentDetails = async () => {
        try {
            // Fetching detailsRequested from the contract and saving it to the state
            const [id, name, age, gmail, mobileNo, course, remarks, photo, detailsRequestedValue] = await Promise.all([
                ...
                studentContract.methods.detailsRequested().call()  // Fetch detailsRequested from the contract
            ]);

            // Save the fetched values in a single object
            const studentData = {
                id,
                name,
                age,
            ...
            setDetailsRequested(detailsRequestedValue);  // Set detailsRequested state from the contract value (CHANGE HERE)
        } catch (error) {
