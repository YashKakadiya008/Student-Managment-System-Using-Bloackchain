...
contract ResultContract {
    ...
    modifier onlyAssignedTeacher(address _student) {
        require(teacherFactory.isTeacher(msg.sender), "Only a teacher can add results");
        _;
    }

    constructor(address _adminAddress, address _studentFactoryAddress, address _teacherFactoryAddress) {
    ...
    function createSemesterResult(string memory _semesterName) public onlyAdmin {
    ...
    function addStudentResult(
        ...
        require(bytes(semesterResults[_semesterId].studentResults[_student].resultHash).length == 0, "Result already exists for this student");

        semesterResults[_semesterId].studentResults[_student] = Result(_resultHash);
        semesterResults[_semesterId].studentAddresses.push(_student); 
        emit addStudentResultevent(msg.sender);
    }

    function getStudentResult(uint256 _semesterId, address _student) public view returns (string memory) {
        require(_semesterId < semesterCount, "Semester does not exist");
        return semesterResults[_semesterId].studentResults[_student].resultHash;
    }

    function getAllStudentsForSemester(uint256 _semesterId) public view returns (address[] memory) {
        require(_semesterId < semesterCount, "Semester does not exist");
    ...
    function verifyResult(
        ...
        return keccak256(abi.encodePacked(semesterResults[_semesterId].studentResults[_student].resultHash)) 
