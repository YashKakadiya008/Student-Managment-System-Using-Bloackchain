...
const ManageSpreadsheets = () => {
  const { account } = useBlockchain(); // Teacher account
  ...
  useEffect(() => {
    const fetchSpreadsheets = async () => {
      try {
        const fetchedSpreadsheets = await getSpreadSheets(account); // Fetch spreadsheets for teacher
  ...
  const handleAddSpreadsheet = async (e) => {
    ...
    try {
      await addSpreadSheet(account, sheetLink, title); // Add spreadsheet for teacher

      // Clear form fields
      setSheetLink('');
      setTitle('');

      // Refresh the spreadsheets after adding a new one
